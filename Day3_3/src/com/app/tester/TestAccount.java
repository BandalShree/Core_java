				...
				try {
					...
					switch (sc.nextInt()) {
						case 1:
							...
							break;
						case 2:
							for(BankAccount bankAcc : accounts.values())
						...
						case 6:
							...
							for(BankAccount bankAcc : sortedAccounts.values())
								System.out.println(bankAcc);
							break;
						case 7:
							List <BankAccount> listAccounts = new ArrayList<>();
							for(BankAccount bankAcc : accounts.values())
								listAccounts.add(bankAcc);
							Collections.sort(listAccounts,new Comparator<BankAccount>() {
								@Override
								public int compare(BankAccount acc1,BankAccount acc2) {
									return acc1.getAccOpenDate().compareTo(acc2.getAccOpenDate());
								}
							});
